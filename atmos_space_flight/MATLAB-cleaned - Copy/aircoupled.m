...
function xdot=aircoupled(t,x)
	...
	udot = -Q0 + q*S*(Cxu*x(9) + Cxa*x(7) + x(5)*(-cos(phi0)*CL + ...
		sin(phi0)*sin(A0)*m*gn/(q*S)))/(m*v0);

	alphadot = (2*x(9)*omega*cos(delta)*sin(A0) + 2*x(9)*v0/(rm + h0) + x(9)*Q0 + ...
		x(2) + q*S*(Czu*x(9) + Cza*x(7) + x(5)*(sin(phi0)*CL-cos(phi0)* ...
		cos(A0)*m*gn/(q*S)) + c2vCzq*x(2))/(m*v0))/(1 - q*S*c2vCzad/(m*v0));
	...
	betadot = -x(3) + v*cos(phi0)^2*cos(A0)*tan(delta)*x(8)/(rm+h0) + ...
		2*omega*cos(delta)*sin(phi0)*sin(A0)*x(8) + g*x(4)*cos(phi0)/v + ...
		gn*(x(4)*sin(phi0)*cos(A0) - x(6)*cos(A0)) + q*S*(Cyb*x(8) + ...
		b*Cyr*x(3)/(2*v))/(m*v);
