...
function xdot = airflight3dof(t, x)
	...
	raddot = x(4)*sfpa;
	veldot = -g*sfpa + gn*cchi*cfpa + Xfo/m + omega*omega*x(3)*cla*(sfpa*cla - ...
		cfpa*cchi*sla);

	gammadot = (x(4)/x(3) - g/x(4))*cfpa - gn*cchi*sfpa/x(4) + Zfo/(x(4)*m) + ...
		2*omega*schi*cla + omega*omega*x(3)*cla*(cfpa*cla + sfpa*cchi*sla)/x(4);

	headdot = x(4)*schi*tan(x(2))*cfpa/x(3) - gn*schi/x(4) - Yfo/(x(4)*cfpa*m) - ...
		2*omega*(tan(x(5))*cchi*cla-sla) + omega*omega*x(3)*schi*sla*cla/(x(4)*cfpa);
