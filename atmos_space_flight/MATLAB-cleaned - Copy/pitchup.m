...
function xdot = pitchup(t, x)
	...
	phi = x(4) - x(3);
	hdot = v*sin(phi);
	udot = -Q0 + q*S*(Cxu*x(2) + Cxa*x(3) + phi*(-cos(phi0)*CL + ...
		sin(phi0)*sin(A0)*m*gn/(q*S)))/(m*v0);
		 
	alphadot = (2*x(2)*omega*cos(delta)*sin(A0) + 2*x(2)*v0/(rm + alt) + ...
		x(2)*Q0+x(5) + q*S*(Czu*x(2) + Cza*x(3) + phi*(sin(phi0)*CL - ...
		cos(phi0)*cos(A0)*m*gn/(q*S)) + c*Czq*x(5)/(2*v0))/(m*v0))/(1 - ...
		q*S*c*Czad/(2*m*v0^2));
