...
function xdot = lateralentry(t, x)
	...
	Xfo = -q*S*CD;
	raddot = v*sfpa;  
	veldot = -g*sfpa + gn*cchi*cfpa + Xfo/m + omega*omega*x(1)*cla*(sfpa*cla - ...
		cfpa*cchi*sla);
			
	betadot = -x(7) + v*cfpa^2*cchi*tan(lat0)*x(3)/x(1) + ...
		2*omega*cla*sfpa*schi*x(3) + g*x(4)*cfpa/v + gn*(x(4)*sfpa*cchi - ...
		x(6)*cchi) + q*S*(Cyb*x(3) + b*Cyr*x(7)/(2*v))/(m*v);
